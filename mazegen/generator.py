...
class MazeGenerator:
    ...
    def _carve_passages(self, start_x: int, start_y: int) -> None:
        ...
        while stack:
            ...
            for dir_name, (dx, dy, wall_here, wall_there) in directions.items():
                nx, ny = cx + dx, cy + dy
                if (
                    0 <= nx < self.width
    ...
    def solve(self, start_x: int, start_y: int, exit_x: int, exit_y: int) -> str:
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                if self.grid[y][x] in dead_ends:
                    ...
                    for d_name, (dx, dy, w_here, w_there) in directions.items():
